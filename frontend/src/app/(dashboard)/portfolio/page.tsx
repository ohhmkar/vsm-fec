...
export default function PortfolioPage() {
  ...
  const summaryCards = [
    ...
    },
    {
      ...
      color: totalPnLDollar >= 0 ? 'text-[var(--accent-green)]' : 'text-[var(--accent-red)]',
    },
    {
    ...
    <PageWrapper>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)]">Your holdings and performance</p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs text-[var(--text-dim)]">{card.label}</span>
                </div>
                <p className={`text-lg font-semibold font-mono tabular-nums ${card.color}`}>
                  {card.value}
                </p>
              </motion.div>
            );
        ...
        {snapshots.length > 1 && (
          <motion.div
            ...
            className="bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-xl p-5"
          >
            <h3 className="text-sm font-medium text-[var(--text-secondary)] mb-4">Portfolio Performance</h3>
        ...
        >
          ...
          {holdings.length === 0 ? (
            <div className="px-5 py-12 text-center">
              ...
              <Link href="/stocks" className="text-sm text-[var(--accent-blue)] hover:underline">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--border-color)] text-[var(--text-dim)] text-xs">
                    ...
                    <th className="text-right py-3 px-4 font-medium hidden sm:table-cell">Mkt Value</th>
                ...
                <motion.tbody variants={listVariants} initial="hidden" animate="show">
                  {holdings.map((h) => {
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-right font-mono tabular-nums">{formatCurrency(h.currentPrice)}</td>
                      <td className="py-3 px-4 text-right font-mono tabular-nums hidden sm:table-cell">
                        {formatCurrency(h.marketValue)}
                      </td>
                      <td className="py-3 px-4 text-right">
                        <div className={`font-mono tabular-nums text-xs ${h.pnl >= 0 ? 'text-[var(--accent-green)]' : 'text-[var(--accent-red)]'}`}>
        ...
        {transactions.length > 0 && (
          ...
          >
            ...
            >
              ...
              {showTransactions ? <ChevronUp size={16} className="text-[var(--text-dim)]" /> : <ChevronDown size={16} className="text-[var(--text-dim)]" />}
            </button>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {transactions.slice(0, 20).map((tx) => (
                          <tr key={tx.id} className="border-b border-[var(--border-color)]/50">
                            ...
                            <td className="py-2.5 px-4 font-mono font-medium text-xs">{tx.ticker}</td>
                            <td className="py-2.5 px-4 text-right font-mono tabular-nums text-xs">{tx.shares}</td>
                            <td className="py-2.5 px-4 text-right font-mono tabular-nums text-xs">{formatCurrency(tx.price)}</td>
                            <td className="py-2.5 px-5 text-right font-mono tabular-nums text-xs font-medium">{formatCurrency(tx.total)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
