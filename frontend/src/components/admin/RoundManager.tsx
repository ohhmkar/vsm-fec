...
const ADMIN_URL =
  (process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080") + "/admin";
    ...
    <div className="w-full max-w-4xl mx-auto p-6 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-2xl shadow-lg mt-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-xs text-[var(--text-secondary)] mt-1">
          Leave empty to effectively start immediately upon saving, or use the
          Start Game button elsewhere.
        </p>
      </div>
          ...
          <label className="flex items-center space-x-3 cursor-pointer group">
            <div className="relative">
              <input
                ...
                onChange={(e) =>
                  setRules({ ...rules, noShortSelling: e.target.checked })
                }
      ...
      <div className="flex flex-col gap-4">
        {msg && (
          <div
            className={`p-3 rounded-xl text-sm font-medium ${msg.type === "error" ? "bg-red-500/10 text-red-500 border border-red-500/20" : "bg-green-500/10 text-green-500 border border-green-500/20"}`}
          >
            {msg.text}
          </div>
        )}
